...
func (c *PostgresCollector) collectStatements(ctx context.Context, ch chan<- prometheus.Metric) {
	...
	for rows.Next() {
		...
		var calls, rowsN, blksHit, blksRead, blksDirtied, tempRead, tempWritten int64
		var totalTime, blkReadTime, blkWriteTime float64
		if err := rows.Scan(&qid, &query, &calls, &totalTime, &rowsN, &blksHit, &blksRead, &blksDirtied, &blkReadTime, &blkWriteTime, &tempRead, &tempWritten); err != nil {
			continue
		}
