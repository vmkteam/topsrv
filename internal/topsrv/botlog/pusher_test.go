...
func TestPusher_DropOnQueueFull(t *testing.T) {
	...
	drop := testutil.ToFloat64(p.eventsTotal.WithLabelValues(stateDropped))
	assert.InDelta(t, 4, enq, 0.5, "queue cap = batchSize*2")
	assert.InDelta(t, 6, drop, 0.5, "remaining events dropped")
}
...
func TestPusher_ReplaySpoolOnStartup(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(stalePath, payload, 0o640))
...
func TestPusher_TrimSpoolByBudget(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(cfg.SpoolDir, 0o750))
	for _, name := range []string{"100-aaaa.ndjson.gz", "200-bbbb.ndjson.gz", "300-cccc.ndjson.gz"} {
		require.NoError(t, os.WriteFile(filepath.Join(cfg.SpoolDir, name), make([]byte, 1024*1024), 0o640))
	}
...
func TestPusher_PermanentFailureDropsBatch(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return testutil.ToFloat64(p.eventsTotal.WithLabelValues(stateDropped)) >= 1
