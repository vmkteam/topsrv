...
| `topsrv_netstat_tcp_in_segs_total` | counter | — | TCP segments received. Denominator for the share of `tcp_in_errs_total` |
| `topsrv_netstat_tcp_out_segs_total` | counter | — | TCP segments sent, retransmits included. Denominator for the retransmit ratio: `100 * rate(tcp_retransmits_total[5m]) / rate(tcp_out_segs_total[5m])`, comparable to the 1–2% industry threshold. Prefer it over `topsrv_network_packets_total{direction="tx"}`, which also counts pure ACKs, UDP, ARP and ICMP and so understates the ratio |
...
## Web-logs (`topsrv_weblog_*`)

Opt-in, and separate from bot-logs in every respect: its own token, its own spool subdir, its own delivery metrics. Emitted when `[WebLogs].Enabled = true`. Where bot-logs ships only lines whose UA matched the built-in table, this stream ships **every** line from the access logs named in `LogPaths` — the UA classifier still runs, but its result travels on the event as `uaMatched` rather than deciding whether the event exists. A scraper on a plain browser UA is invisible to the bot stream; that gap is what this one closes.

Both streams can run at once on the same host. They share one parse of each line (the collector parses it for `topsrv_nginx_*` regardless) and attach as two observers to the same tail, so a second stream costs filtering and delivery, not a second read of the file.

**Volume.** One event per request, so expect three to four orders of magnitude more than bot-logs. The local filters are what make that affordable, and they run cheapest-first: wrong file → never reached a backend → wrong backend → excluded prefix. On a front node serving bundles, images and static files from disk, `RequireUpstream` alone typically removes most of the traffic.

Delivery metrics mirror the bot-log set exactly — `topsrv_weblog_events_total{state,reason}`, `_send_errors_total{kind}`, `_batch_duration_seconds`, `_queue_depth`, `_spool_files`, `_spool_bytes` — with the same semantics and the same alerting guidance; substitute the prefix. Observer-side metrics are the stream's own:

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `topsrv_weblog_filtered_total` | counter | reason | Lines rejected before an event was built. `reason` ∈ {`path` (line came from a file this stream did not ask for), `upstream` (`RequireUpstream` and the request never reached a backend), `proxy_host` (`$proxy_host` outside the `Upstreams` allowlist), `prefix` (raw path matched `ExcludePathPrefixes`)}. |
| `topsrv_weblog_match_total` | counter | family | Lines whose UA matched a bot family. A signal, not a filter — these events ship like any other, with `uaMatched=true`. Compare against `topsrv_botlog_match_total` on the same host: the two count the same lines, so a divergence means the streams are reading different files. |
| `topsrv_weblog_tailed_paths` | gauge | — | Access logs actually tailed after the startup format checks. A value below the number of configured `LogPaths` means at least one was refused; the `weblog_*` config warning says which and why. |

`filtered_total` earns its place three times over: a zero count on a filter you configured means the rule never matches (usually written against the normalized path instead of the raw one); its sum with delivered events is what the stream would cost unfiltered, which is how the price of widening it gets estimated without widening it; and it partially stands in for the "this client never fetches assets" signal that disappears along with the filtered lines — partially, because the counter is per host while the signal is per client.

Event payload notes (in addition to the bot-log fields above):

- `uaMatched` / `uaListVersion` — whether the UA matched, and the version of the list that decided. The boundary between the two streams moves when the list changes, so "not a bot" and "the list did not know it yet" have to stay distinguishable on the receiving side.
- `reachedUpstream` / `proxyHost` — whether the request reached a backend, and which one. `proxyHost` needs `$proxy_host` in the log_format; nginx's `-` placeholder ships as empty rather than as a literal dash.
- `requestId` — nginx's `$request_id` (32 hex), or `$http_x_request_id` when an edge assigned it. The one field that joins an event to the backend's logs for the same request. Absent from the format, the receiver generates one: unique, but it joins to nothing.
- `upstreamStatus` — what the backend answered, as opposed to `status`, which is what the client got. They diverge exactly where it matters — nginx serving its own 502, a cached 200 over a dead backend. On a retry nginx logs the whole chain (`"502, 200"`) and the last entry ships, being the one that reached the client.
- `platform` / `appVersion` / `visitorId` — read from `$http_platform`, `$http_version` and the userid module's `$uid_got` (falling back to `$uid_set`) when the format carries them. `visitorId` is the only stable identity above the address: one client rotating addresses under a single cookie and many cookies behind one address are opposite cases and tell apart by nothing else. All three are typed fields rather than `ExtractFields` slots — `MaxExtras` is a hard budget shared with operator `ExtraLabels`, and on a host already logging a few extras there is not room for three more.

**Startup format checks.** Before any path is tailed its `log_format` is read, and a refusal is fatal for that path alone. A format carrying `$request_body`, `$http_cookie` or `$http_authorization` is refused outright — request bodies, cookies and credentials are never shipped, and the check is not "filter them out later" because a config can declare such a format without attaching it anywhere, leaving it one line away from a vhost that is tailed. A format with no client address, no `$status` or no URI variable is refused as unusable. Missing timestamp, verb or `$upstream_response_time` only warn. Each raises `topsrv_collector_config_warnings_total{kind="weblog_*"}` — see the config-warnings row below for the full list.
...
| `topsrv_collector_config_warnings_total` | counter | kind | Operator-config warnings raised at startup. `kind` ∈ {`high_card_label` (denylisted variable in ExtraLabels), `missing_extract` (ExtraLabels references variable absent from ExtractFields → empty label values), `truncated_extract` (ExtractFields > MaxExtras=8 → tail dropped), `botlog_no_ua_field` (BotLogs enabled but no log_format carries http_user_agent — override via `[BotLogs.FieldAliases].UserAgent`), `botlog_no_time_field` (no log_format carries a request timestamp → events stamped with the agent clock, see `ts` above), `botlog_no_method_field` (no log_format carries the request verb → `method` ships empty), `botlog_alias_mismatch` (tailed paths resolve to non-identical botlog aliases; first path's resolution is used), `weblog_unknown_path` (a `[WebLogs].LogPaths` entry has no matching access_log — check the exact path in `nginx -T`), `weblog_unsafe_format` (format carries `$request_body`/`$http_cookie`/`$http_authorization` → path not tailed), `weblog_no_client_ip` and `weblog_incomplete_format` (format lacks a client address, `$status` or a URI variable → path not tailed), `weblog_no_timestamp`, `weblog_no_method`, `weblog_no_upstream`, `weblog_no_request_id`, `weblog_no_upstream_status` (degraded but still tailed), `weblog_upstream_filter_disabled` (`Upstreams` set but the format has no `$proxy_host`; the filter is ignored rather than dropping every line)}. Any non-zero value points at a fixable config — see the matching WARN in stdout/journald for the offending names |
